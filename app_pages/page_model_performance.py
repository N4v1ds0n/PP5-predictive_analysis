...
def page_ml_performance_body():
    ...
    st.info("Explore the model’s performance across metrics and visualizations.")
    ...
    pca_df = load_csv(f"outputs/{version}/pca_results.csv", error_msg="PCA data not found.")
    if pca_df is not None:
        center_component(lambda: st.plotly_chart(load_drt_figure(pca_df)))
        center_component(lambda: st.info(
            "- 📌 Partial class separation suggests overlapping but distinguishable features."
        ))
    ...
    tsne_df = load_csv(f"outputs/{version}/tsne_results.csv", error_msg="t-SNE data not found.")
    if tsne_df is not None:
        center_component(lambda: st.plotly_chart(load_drt_figure(tsne_df)))
        center_component(lambda: st.info(
            "- 📌 t-SNE reveals better-defined local groupings, especially for the healthy class, which appears more compact and clustered toward the lower part of the graph."
        ))
    ...
    umap_df = load_csv(f"outputs/{version}/umap_results.csv", error_msg="UMAP data not found.")
    if umap_df is not None:
        center_component(lambda: st.plotly_chart(load_drt_figure(umap_df)))
        center_component(lambda: st.info(
            "- 📌 The 'diseased' and 'healthy' samples form distinct clusters, with minimal overlap. This implies your features contain highly discriminative information."
        ))
    ...
    train_report = load_csv(f"outputs/{version}/classification_report_train.csv")
    test_report = load_csv(f"outputs/{version}/classification_report_test.csv")
    ...
    with col1:
        display_image_centered(f"outputs/{version}/confusion_matrix_test.png", "Test Set Confusion Matrix", width=350)
    with col2:
        display_image_centered(f"outputs/{version}/confusion_matrix_train.png", "Train Set Confusion Matrix", width=350)
    with col3:
        display_image_centered(f"outputs/{version}/confusion_matrix_val.png", "Validation Set Confusion Matrix", width=350)
    ...
    col1, col2 = st.columns(2)
    with col1:
        st.image(f"outputs/{version}/model_training_acc.png", caption="Accuracy Over Epochs", use_container_width=True)
    with col2:
        st.image(f"outputs/{version}/model_training_losses.png", caption="Loss Over Epochs", use_container_width=True)
    ...
    display_image_centered(f"outputs/{version}/roc_curve.png", "ROC Curve", width=700)

    center_component(lambda: st.info(
        "- 🟢 AUC = 1 → perfectdiscriminative ability."
    ))
